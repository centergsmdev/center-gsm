...
const duplicateError = (code?: string) =>
  code === "23505"
    ? "Bu slug daha önce kullanılmış. Benzersiz bir slug girin."
    : SAFE_ERROR;
const HEADER_LIMIT_ERROR = "Üst menüde en fazla 7 kategori gösterebilirsiniz.";
const orderMap = {
...
async function productCounts(
  ...
  const result = await client.from("products").select("category_id,brand_id");
...
export async function getAdminCategories(
...
): Promise<AdminProductResult<AdminCategory[]>> {
  const client = connection();
  if (!client)
  ...
  const [column, ascending] = orderMap[filters.sort];
  const [rows, counts] = await Promise.all([
    query.order(column, { ascending }),
    productCounts("category_id"),
  ]);
...
export async function getAdminBrands(
  filters: AdminTaxonomyFilters,
): Promise<AdminProductResult<AdminBrand[]>> {
  const client = connection();
  if (!client)
    return { data: null, error: "Supabase bağlantısı yapılandırılmamış." };
...
export async function createAdminCategory(
...
): Promise<AdminProductResult<Tables<"categories">>> {
  const client = connection();
  if (!client)
    return { data: null, error: "Supabase bağlantısı yapılandırılmamış." };
  if (values.show_in_header && values.is_active) {
    const selected = await client
      .from("categories")
      .select("id", { count: "exact", head: true })
      .eq("show_in_header", true)
      .eq("is_active", true);
    if (selected.error) return { data: null, error: SAFE_ERROR };
    if ((selected.count ?? 0) >= 7)
      return { data: null, error: HEADER_LIMIT_ERROR };
  }
  const result = await client
    ...
    .insert({
      ...
      show_in_header: values.show_in_header ?? false,
    })
    .select("*")
    .single();
  return result.error
...
export async function deactivateAdminCategory(id: string) {
  return deactivate("categories", id);
}

export async function createAdminBrand(
  values: TaxonomyFormValues,
): Promise<AdminProductResult<Tables<"brands">>> {
  const client = connection();
  if (!client)
    return { data: null, error: "Supabase bağlantısı yapılandırılmamış." };
  const result = await client
...
export async function deactivateAdminBrand(id: string) {
  return deactivate("brands", id);
}
