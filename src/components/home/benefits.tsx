...
export function Benefits() {
  return (
    <section
      ...
      className="reveal-on-scroll pb-7 sm:pb-16"
    >
      <Container>
        <div className="stagger-grid grid gap-2.5 sm:grid-cols-2 sm:gap-3 lg:grid-cols-4 lg:gap-4">
          {benefits.map(({ title, description, icon: Icon }) => (
            <article
              key={title}
              className="home-premium-interactive home-premium-surface group border border-zinc-200/80 bg-white p-4 sm:min-h-64 sm:p-7"
            >
