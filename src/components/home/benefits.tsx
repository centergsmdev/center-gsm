        ...
        <StaggerContainer className="grid gap-2.5 sm:grid-cols-2 sm:gap-3 lg:grid-cols-4 lg:gap-4">
          {benefits.map(({ title, description, icon: Icon }) => (
            <AnimatedCard key={title} className="h-full">
              <article className="home-premium-interactive home-premium-surface group h-full border border-zinc-200/80 bg-white p-3 sm:min-h-64 sm:p-7">
                <span className="grid size-10 place-items-center rounded-xl bg-zinc-950 text-white shadow-[0_10px_24px_rgba(9,9,11,0.16)] transition-[transform,background-color,box-shadow] duration-300 ease-premium group-hover:scale-105 group-hover:bg-primary group-hover:shadow-[0_16px_36px_rgba(220,38,38,0.28)] sm:size-16 sm:rounded-2xl sm:shadow-[0_14px_30px_rgba(9,9,11,0.18)]">
                  <Icon
                ...
                <h3 className="mt-3 text-sm font-black tracking-[-0.035em] text-zinc-950 sm:mt-8 sm:text-xl">
                  {title}
                </h3>
                <p className="mt-1 text-[11px] leading-4 text-zinc-500 sm:mt-3 sm:text-sm sm:leading-6">
                  {description}
                </p>
              </article>
            </AnimatedCard>
