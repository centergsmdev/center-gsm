...
begin
  ...
  if jsonb_typeof(v_pricing->'coupon') = 'object' then insert into public.coupon_usages(coupon_id,user_id,order_id) values((v_pricing->'coupon'->>'id')::uuid,auth.uid(),v_order_id); end if;
