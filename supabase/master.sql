...
create table if not exists public.admin_ui_preferences (
  ...
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
  constraint admin_ui_preferences_menu_order_array check (jsonb_typeof(menu_order) = 'array'),
  constraint admin_ui_preferences_menu_presets_object check (jsonb_typeof(menu_presets) = 'object'),
  constraint admin_ui_preferences_activity_seen_object check (jsonb_typeof(activity_seen_at) = 'object')
);
drop trigger if exists set_admin_ui_preferences_updated_at on public.admin_ui_preferences;
create trigger set_admin_ui_preferences_updated_at before update on public.admin_ui_preferences for each row execute function public.set_updated_at();
...
drop policy if exists "Admins manage own UI preferences" on public.admin_ui_preferences;
create policy "Admins manage own UI preferences" on public.admin_ui_preferences for all to authenticated
